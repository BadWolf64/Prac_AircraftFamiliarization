...
bad_LandingFunction = {
	...
	if (vehicle player isKindOf "Helicopter") then{ 
		...
		private _indexLZ = round random[1,5,15];
...
bad_setUpLZFunction = {
	...
	if (_opposition) then {
		...
		{_x setSkill 0.35;} foreach units _groupAI;
		{_x setSkill 0.35;} foreach units _groupAI2;
...
bad_CleanUpAO = {
	...
	_listObjectsLZ = _posMarker nearObjects["SmokeShell", 500];
	_ListObjectsAO = _posMarker nearObjects["Man", 1000];
